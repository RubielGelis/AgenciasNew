...
console.log('Inspeccionando inserción en CotizacionServiciosFormasPago...');
...
const res = await req.query(`
  BEGIN TRY
    ...
    -- Declaramos tablas temporales identicas al SP
    ...
    SELECT 1, NULL, C.node.value('cd_consecutivo[1]','CHAR(8)'), 1, GETDATE(), GETDATE(), 1, GETDATE(), '0000001091', 'TEST', '0000001091', 'TEST', 'DIR', '', '', '', '', '', '', 1, 1, '000', 1, 1, 1, '', '', '', 1, GETDATE(), 0, 0, 0, 0, '', '', 0, '', '', 0, 65, '', 1, 23138
    ...
    -- 4. Actualizar id_cotizacion y bl_existe de @Cotizacion (como lo hace el SP)
    -- En este caso bl_existe ya es 1.

    -- 5. Mapear id_CotizacionServicios
		UPDATE CCS
		...
		INNER JOIN @Cotizacion C ON C.cd_consecutivo = CCS.cd_Cotizacion;

    -- 6. Parsear formas de pago desde XML
    ...
    -- 7. Resolver FKs en formas de pago (lógica que quitó bl_existe=0)
		UPDATE FP
		SET FP.id_CotizacionServicios = CS.id_CotizacionServicios, -- Usamos el resuelto en @CotizacionServicios
		...
		INNER JOIN @Cotizacion C ON C.cd_consecutivo = FP.cd_Cotizacion;

    SELECT 
      'Resultado final' AS Step,
      FP.id_CotizacionServicios,
      FP.Id_Cotizacion,
      FP.id_FormasPago,
      FP.ds_FPnm,
      FP.am_valor,
      CS_REAL.id AS ID_Real_Servicio
    FROM @CotizacionServiciosFormasPago FP
    LEFT JOIN dbo.CotizacionServicios CS_REAL ON CS_REAL.id = FP.id_CotizacionServicios;
  ...
  END CATCH
`);
console.log('Resultado:', JSON.stringify(res.recordset, null, 2));
